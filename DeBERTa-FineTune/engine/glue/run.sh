...
CUDA_VISIBLE_DEVICES=4,5,6,7 OMP_NUM_THREADS=1 TOKENIZERS_PARALLELISM=true MPLBACKEND='Agg' nohup accelerate launch run_glue.py --task_name $TASK --model_type microsoft/$MODEL --train_file $TRAIN_FILE --train_batch_size $TRAIN_BS --val_file $VAL_FILE --val_batch_size $VAL_BS --epochs $EPOCHS --warmup_steps $WARMUP_STEPS --lr $LR --optimizer $OPTIMIZER --lr_scheduler_type $SCHEDULER --max_seq_length 320 --pad_to_max_seq_length --cls_dropout 0.2 --weight_decay $WEIGHT_DECAY --pruning --prune_frequency $PRUNE_FREQ --sparse_steps $SPARSE_STEPS --prune_deploy_device $PRUNE_DEPLOY_DEVICE --kd_on --teacher_path $TEACHER --teacher_init $TEACHER_INIT >> $NOHUP_OUTPUT 2>&1 &
