...
from data.process import generate_features
...
if __name__ == '__main__':
    ...
    best_f1 = best_em = 0.
    best_checkpoint_dir = os.path.join(config.OUTPUT, f'{config.MODEL.NAME}-{config.TAG}-best')
    ...
    if config.TRAIN.AUTO_RESUME:
        ...
        if resume_file:
            ...
            best_f1, best_em = load_checkpoint(model, optimizer, lr_scheduler, config, logger)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        if not epoch % config.SAVE_FREQ or epoch == config.TRAIN.EPOCHS - 1:
            ...
            checkpoint = save_checkpoint(
                checkpoint_dir, model, optimizer, lr_scheduler, epoch, config,
                best_f1, best_em
            )
        ...
        if f1 + em > best_f1 + best_em:
            best_checkpoint = save_checkpoint(
                best_checkpoint_dir, model, optimizer, lr_scheduler, 
